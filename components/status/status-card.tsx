  ...
  return (
    ...
    >
      <Card className="glass-panel border-border/50 bg-card/40 dark:bg-card/25">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto_auto_auto] items-center gap-x-2 gap-y-1 px-3 py-2.5 sm:gap-x-3 sm:px-3.5 sm:py-3">
          ...
          <span className="w-[3.25rem] shrink-0 text-right text-xs tabular-nums text-muted-foreground sm:w-16">
            {formatLatencyMs(service.latency)}
          </span>
          <span className="w-[3.25rem] shrink-0 text-right text-xs font-medium tabular-nums text-foreground/85 sm:w-16">
          ...
          <div className="flex w-[4.5rem] shrink-0 items-center justify-end gap-1.5 sm:w-auto sm:min-w-[5.5rem]">
            ...
            <StatusBadge status={service.status} className="px-2 py-0.5" />
          </div>
        </div>
      </Card>
    </motion.div>
